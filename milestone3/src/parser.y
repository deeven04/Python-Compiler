...
void enter_symbol(symbol_table*current,char token[1000],char type[1000],int line){
    ...
    if(type!="function" && type!="class"){
        ...
        int size = get_siz(type);
        entry->offset = offset;
        offset +=size;
        entry->size = size;
        cout <<"offset:       " << offset<<","<<entry->offset<<endl;
...
void print(symbol_table*temp){
    ...
    if(temp!=NULL){
        string filen = temp->token;
        string filename = filen + ".csv"; csv++;
...
string get_offset(symbol_entry* entry,string name, string func){
...
vector<int> for_if_stack;
vector<int> loop_stack;
vector<int> curr_num;

int loop_stack_num = 0;
vector<tuple<string,string,string,string,string>> func_types;
%}
...
%type<n_terminal> op_term AddOrSub funcdef_def classdef_def if_test for_stmt_begin
...
funcdef: funcdef_def suite {current=current->parent; emitt("","","","end",-1,backtrue[(backtrue.size())-1]); arguments.clear();}
    | funcdef_def newline_plus suite {current=current->parent;emitt("endfunc","","","end",-1,backtrue[(backtrue.size())-1]); arguments.clear();} 
    ;

funcdef_def: DEF NAME parameters COLON {
        
        cout<<"def2"<<endl;
        char str[1000];
        strcpy(str,"function");
        cout<<"enter_symbol2"<<endl;
        enter_symbol(current,($2)->token,str,yylineno);
        cout<<"symbol_table2"<<endl;
        strcpy(current->type,"function");
        current=func_table_lookup(current,($2)->token);//changed this from creating table
        $2->declared=true;
        cout<< "def done" << endl;

        // 3ac
        
        emitt("begin",$2->token,"","",-1,backtrue[(backtrue.size())-1]);
        $$->i_number = inst_num-1;
       ...
       for(int i=0;i<arguments.size();i++) { 
        ...
        symbol_entry* temp_entry = entry_lookup(current,char_array);
        cout << "\n" << "\n" << "\n"<<temp_entry->offset<<temp_entry->token<<"\n"<<"fejiefjif"<<endl;
        emitt("","popparam","",temp_entry->temp_var,-1,backtrue[(backtrue.size())-1]);
        }
        //strcpy($$->offset,current->offset);
        arguments.clear();
    }
    |  DEF NAME parameters RIGHT_ARROW test COLON {
        cout<<"def1"<<endl;
        char str2[1000];
        strcpy(str2,"function");
        strcpy(current->type, str2);
        if(($2)->token=="_init_") {//use this if error occurs in type
         if(current->type!="class"){
          cout<<"func _init_ cannot be in class declaration at line no:"<<yylineno<<endl;
         exit(EXIT_FAILURE);
         }
        }
        ...
        if((current->children).find($5->token)==(current->children).end()){
          cout<<"Return type not present at line no:"<<yylineno<<endl;
          exit(EXIT_FAILURE);
        } 
          strcpy(current->return_type,$5->type);
        */
          emitt("begin",$2->token,"","",-1,backtrue[(backtrue.size())-1]);
        ...
        cout << "\n" << "\n" << "\n"<<temp_entry->offset<<temp_entry->token<<"\n"<<"fejiefjif"<<endl;
        emitt("","popparam","",temp_entry->temp_var,-1,backtrue[(backtrue.size())-1]);
        emitt("",temp_entry->temp_var,"",get<0>(arguments[i]),-1,backtrue[(backtrue.size())-1]);}
        arguments.clear();
    }
...
tfpdef: NAME COLON DATA_TYPE {
        ...
        symbol_entry* temp_entry = entry_lookup(current,$1->token);
        string t = new_temporary();
        strcpy(temp_entry->temp_var,t.c_str());
        arguments.push_back(make_tuple($1->token,t.c_str(),$3->type,0,0));
        }
    | NAME COLON NAME LEFT_SQUARE_BRACE DATA_TYPE RIGHT_SQUARE_BRACE{
            if(current->table.find($1->token)==current->table.end()){
            cout<<"enter_symbol5"<<endl;
            cout<<"tfpdef test type:"<<$3->type<<endl;
            strcpy($1->type,$5->type);
            char char_array[1000];
    ...
    | NAME {arguments.push_back(make_tuple($1->token,"",$1->type,0,0));cout<<"tfpdef2"<<endl; }
...
flow_stmt: break_stmt { cout<<"flow_stmt1"<<endl;  }
    ...
    ;
break_stmt: BREAK {cout<<"break_stmt1"<<endl; emitt("","","","break",-1,backtrue[(backtrue.size())-1]);}
    ;
...
return_stmt: RETURN  {cout<<"return_stmt1"<<endl;}
    ...
    | RETURN expr { 
      ...
      emitt("",$2->temp_var,"","return",-1,backtrue[(backtrue.size())-1]); $$->i_number=$2->i_number;
    }
    ;
...
compound_stmt: if_stmt {cout<<"compound_stmt1"<<endl;}
    ...
    ;
print_stmt : PRINT LEFT_PAR arglist RIGHT_PAR {cout<<"print_stmt1"<<endl;emitt("",$3->temp_var,"","print",-1,backtrue[(backtrue.size())-1]);}
    ;
assignment_stmt : atom_expr EQUAL_TO expr {
        ...
        string p = $3->type;
        if(p=="boolean"){
        emitt("","true","",$1->token,-1,backtrue[(backtrue.size())-1]);
        emitt("","false","",$1->token,-1,backtrue[(backtrue.size())-1]);
        }else{
        emitt("",$3->temp_var,"",$1->temp_var,-1,backtrue[(backtrue.size())-1]);
        };
    ...
    | NAME COLON NAME EQUAL_TO atom {
        ...
        else {cout<<"redeclared at line no: "<<yylineno<<endl;exit(EXIT_FAILURE);}
        cout << "new_rule" << endl;}
    | NAME COLON NAME LEFT_SQUARE_BRACE DATA_TYPE RIGHT_SQUARE_BRACE EQUAL_TO atom {
        ...
        array_func($3->token,$5->type);
                
        cout << $8->temp_var <<endl;
        emitt("",$8->temp_var,"",$1->token,-1,backtrue[(backtrue.size())-1]);
        symbol_entry* temp_entry = entry_lookup(current,$1->token);
        strcpy(temp_entry->temp_var,$8->temp_var); 
        };
    }
    ...
    | NAME COLON DATA_TYPE EQUAL_TO atom_expr{
        ...
        symbol_entry* temp_entry = entry_lookup(current,char_array);
        cout << "\n" << "\n" << "\n"<<temp_entry->offset<<temp_entry->token<<"\n"<<"fejiefjif"<<endl;
        emitt("",$5->temp_var,"",$1->token,-1,backtrue[(backtrue.size())-1]);
        strcpy(temp_entry->temp_var,$5->temp_var);
        };
    }
    | NAME DOT NAME COLON DATA_TYPE EQUAL_TO atom {
       ...
       $$->i_number = $3->i_number;
        ...
        emitt("",new_temporary(),"",$1->token,-1,backtrue[(backtrue.size())-1]);
        }; 
    }
    ;
if_test: IF test {
     loop_stack_num++;
        loop_stack.push_back(loop_stack_num);
        string s1 = ".L"+to_string(loop_stack_num);
    emitt("",s1,"","loop:",-1,backtrue[(backtrue.size())-1]);
    string s2 = "goto end.L" + to_string(loop_stack_num);
    string s3 = $2->temp_var;
    emitt("if",s3,s2,"",-1,backtrue[(backtrue.size())-1]);

}
if_stmt: if_test COLON suite a ELSE COLON suite {cout<<"if_stmt1"<<endl;string s1 = "end.L"+to_string(loop_stack[(loop_stack.size()-1)]); loop_stack.pop_back(); emitt("",s1,"","end loop",-1,backtrue[(backtrue.size())-1]);}
    |if_test COLON suite {  cout<<"if_stmt2"<<endl;
        string s2 = "end.L" + to_string(loop_stack[(loop_stack.size())-1]+1);
         emitt("",s2,"","goto",-1,backtrue[(backtrue.size())-1]);
        string s1 = "end.L"+to_string(loop_stack[(loop_stack.size()-1)]);
         loop_stack.pop_back(); emitt("",s1,"","end loop",-1,backtrue[(backtrue.size())-1]);
         }
     ELSE COLON {
        loop_stack_num++;
        loop_stack.push_back(loop_stack_num);
    ...
    | if_test COLON suite a {cout<<"if_stmt3"<<endl;string s1 = "end.L"+to_string(loop_stack[(loop_stack.size()-1)]); loop_stack.pop_back(); emitt("",s1,"","end loop",-1,backtrue[(backtrue.size())-1]);}
    | if_test COLON suite {cout<<"if_stmt4"<<endl;string s1 = "end.L"+to_string(loop_stack[(loop_stack.size()-1)]); loop_stack.pop_back(); emitt("",s1,"","end loop",-1,backtrue[(backtrue.size())-1]);}
    ;
...
while_stmt: WHILE test COLON suite ELSE COLON suite {cout<<"while_stmt1"<<endl;}
    ...
    ;
for_stmt_begin: FOR exprlist IN testlist {cout<<"for_stmt1"<<endl;
        if(strcmp($2->type, $4->type)) {
            cout << "arguments in for loop are different, line no: " << yylineno << endl;
        }}
    | FOR exprlist IN RANGE LEFT_PAR testlist RIGHT_PAR { cout<<"for_stmt3"<<endl;

         if(strcmp($2->type, $4->type)) {
            cout << "arguments in for loop are different, line no: " << yylineno << endl;
        }
        for_if_stack.push_back(inst_num-2);
        curr_num.push_back(curr_inst_num);
        backtrue.push_back(1);
        loop_stack_num++;
        loop_stack.push_back(loop_stack_num);
        string s1 = ".L"+to_string(loop_stack_num);
        emitt("",s1,"","loop:",-1,backtrue[(backtrue.size())-1]);
        
         if(backtrue[backtrue.size()-2] == 1){
        for_if_stack.push_back(inst_num+curr_inst_num);
        }
        else{
            for_if_stack.push_back(inst_num-1+curr_inst_num);
        }
       
        strcpy($$->temp_var,$2->temp_var);
        strcpy($$->temp_op,$6->temp_var);
        
    }
for_stmt:for_stmt_begin COLON suite {   backtrue.pop_back(); backpatch($1->temp_var,$1->temp_op,">",inst_num-1,curr_num[(curr_num.size())-1],backtrue[(backtrue.size())-1],loop_stack[(loop_stack.size())-1]); curr_num.pop_back(); emitt("+",$1->temp_var,"1",$1->temp_var,-1,backtrue[(backtrue.size())-1]); string s5 = ".L"+to_string(loop_stack[(loop_stack.size())-1]); emitt("",s5,"","goto",for_if_stack[(for_if_stack.size())-1],backtrue[(backtrue.size())-1]); for_if_stack.pop_back();string s1 = "end.L"+to_string(loop_stack[(loop_stack.size()-1)]); loop_stack.pop_back(); emitt("",s1,"","end loop",-1,backtrue[(backtrue.size())-1]);}
/*
...
test: or_test IF or_test ELSE test{
    ...
    | or_test {
        cout<<"test2"<<endl;
        strcpy($$->type,$1->type);
        strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;strcpy($$->type,$1->type);}
    ;
or_test: and_test e {  
    cout << "or_test1" << endl;
    if(allowed_types($1->type,$2->type)){
        ...
        strcpy($$->temp_op,$1->temp_op); 
        strcpy($$->temp_var,$1->temp_var);
        }
    ;
e:            {cout << "e1" << endl;strcpy($$->temp_op,"null");}
    | OR and_test e{
        cout << "e2" << endl;
       strcpy($$->type,$1->type);
       string t = new_temporary(); emitt(t,$2->temp_var,$3->temp_var,$3->temp_op,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,"OR"); strcpy($$->temp_var,t.c_str());$$->i_number = $2->i_number;
    }
    ;
and_test: not_test f  { 
    cout << "and_test1" << endl;
    if($2 && allowed_types($1->type,$2->type)){
                strcpy($$->type, $2->type);
        }else if (!(strcmp($2->type,""))) {
            strcpy($$->type, $1->type);
        }
    ...
    ;
f:            {cout << "f1" << endl;}
    | AND not_test f        {cout << "f2" << endl;strcpy($$->type,$2->type);strcpy($$->temp_var,$2->temp_var);}
    ;
not_test: NOT not_test    {cout << "not_test1" << endl;strcpy($$->type,$2->type); string t = new_temporary(); string s = "not"; string s1 = $2->temp_var; string s3 = s+" "+s1;  strcpy($$->temp_var,s3.c_str()); strcpy($$->temp_op,"not");$$->i_number = $2->i_number;
            
            }
    | comparison          {cout << "not_test2" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var); $$->i_number = $1->i_number;}
    ; 
comparison: expr g {
    ...
    if($2->if_null == 0){string t = new_temporary(); string s2 = " "; string s1= $1->temp_var + s2 + $2->temp_op + s2  + $2->temp_var; strcpy($$->temp_var,s1.c_str());
     //emitt($2->temp_op,$1->temp_var,$2->temp_var,t.c_str(),-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_var,t.c_str()); $$->i_number = $1->i_number;
     }
    else{strcpy($$->temp_var,$1->temp_var);}
    }
    ;
g:             {
    char str[1000];
    ...
    | comp_op expr g {cout << "g2" << endl;strcpy($$->type,$2->type);strcpy($$->temp_op,$1->temp_op); strcpy($$->temp_var,$2->temp_var);}
    ;
comp_op: LT       {cout << "comp_op1" << endl;strcpy($$->temp_op,">=");}
    |GT           {cout << "comp_op2" << endl;strcpy($$->temp_op,"<=");}
    |EQUAL_EQUAL  {cout << "comp_op3" << endl;strcpy($$->temp_op,"!=");}
    |GTE          {cout << "comp_op4" << endl;strcpy($$->temp_op,"<");}
    |LTE          {cout << "comp_op5" << endl;strcpy($$->temp_op,">");}
    |LT GT        {cout << "comp_op6" << endl;strcpy($$->temp_op,"<>");}
    |EXEQ         {cout << "comp_op7" << endl;strcpy($$->temp_op,"==");}
    |IN           {cout << "comp_op8" << endl;strcpy($$->temp_op,"not in");}
    |NOT IN       {cout << "comp_op9" << endl;strcpy($$->temp_op,"in");}
    |IS           {cout << "comp_op10" << endl;strcpy($$->temp_op,"not is");}
    |IS NOT       {cout << "comp_op11" << endl;strcpy($$->temp_op,"is");}
    ;
    
expr: xor_expr         {cout << "expr1" << endl;strcpy($$->type,$1->type); strcpy($$->temp_var,$1->temp_var); $$->i_number = $1->i_number;}
  | xor_expr or_xor     {cout << "expr2" << endl;if(allowed_types($1->type,$2->type)){strcpy($$->type,$1->type);  string t = new_temporary(); emitt($2->temp_op,$1->temp_var,$2->temp_var,t,-1,backtrue[(backtrue.size())-1]);strcpy($$->temp_var,t.c_str());$$->i_number = $1->i_number;} else{cout<<"error"<<endl;}}
  ;

or_xor: BITWISE_OPER_OR xor_expr    {cout << "or_xor1" << endl;strcpy($$->type,$2->type);strcpy($$->temp_var,$2->temp_var); strcpy($$->temp_op,"bitwise_or");$$->i_number = $2->i_number;}
  | BITWISE_OPER_OR xor_expr or_xor       {cout << "or_xor2" << endl;if(allowed_types($2->type,$3->type)){strcpy($$->type,$2->type);string t = new_temporary(); emitt($3->temp_op,$2->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,"bitwise_or"); strcpy($$->temp_var,t.c_str());$$->i_number = $2->i_number; } else{cout<<"error"<<endl;}}
  ;

xor_expr: and_expr           {cout << "xor_expr1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;}    
  | and_expr xor_and         {cout << "xor_expr2" << endl;if(allowed_types($1->type,$2->type)){strcpy($$->type,$1->type); string t = new_temporary(); emitt($2->temp_op,$1->temp_var,$2->temp_var,t,-1,backtrue[(backtrue.size())-1]);strcpy($$->temp_var,t.c_str());$$->i_number = $1->i_number;} else{cout<<"error"<<endl;}}
  ;

xor_and: BITWISE_OPER_XOR and_expr      {cout << "xor_and1" << endl;strcpy($$->type,$2->type);strcpy($$->temp_var,$2->temp_var); strcpy($$->temp_op,"bitwise_xor");$$->i_number = $2->i_number;}
  | BITWISE_OPER_XOR and_expr xor_and       {cout << "xor_and2" << endl;if(allowed_types($2->type,$3->type)){strcpy($$->type,$2->type);string t = new_temporary(); emitt($3->temp_op,$2->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,"bitwise_xor"); strcpy($$->temp_var,t.c_str()); $$->i_number = $2->i_number;} else{cout<<"error"<<endl;}}
  ;

and_expr: shift_expr          {cout << "and_expr1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;}       
  | shift_expr and_shift        {cout << "and_expr2" << endl;if(allowed_types($1->type,$2->type)){strcpy($$->type,$1->type);string t = new_temporary(); emitt($2->temp_op,$1->temp_var,$2->temp_var,t,-1,backtrue[(backtrue.size())-1]);strcpy($$->temp_var,t.c_str());$$->i_number = $1->i_number;} else{cout<<"error"<<endl;}}
  ;

and_shift: BITWISE_OPER_AND shift_expr       {cout << "and_shift1" << endl;strcpy($$->type,$2->type);strcpy($$->temp_var,$2->temp_var); strcpy($$->temp_op,"&&");$$->i_number = $2->i_number;}
  | BITWISE_OPER_AND shift_expr and_shift    {cout << "and_shift2" << endl;if(allowed_types($2->type,$3->type)){strcpy($$->type,$2->type);string t = new_temporary(); emitt($3->temp_op,$2->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,"&&"); strcpy($$->temp_var,t.c_str());$$->i_number = $2->i_number;} else{cout<<"error"<<endl;}}
  ;

shift_expr: arith_expr      {cout << "shift_arith1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;}
  | arith_expr shift_arith  {cout << "shift_arith2" << endl;
  if(allowed_types($1->type,$2->type)){strcpy($$->type,$1->type);string t = new_temporary(); emitt($2->temp_op,$1->temp_var,$2->temp_var,t,-1,backtrue[(backtrue.size())-1]);strcpy($$->temp_var,t.c_str());$$->i_number = $1->i_number;} else{cout<<"error"<<endl;}}
  ;

shift_arith: shift arith_expr       {cout << "shift_arith1" << endl;strcpy($$->type,$2->type);strcpy($$->temp_var,$2->temp_var); strcpy($$->temp_op,$1->temp_op);$$->i_number = $2->i_number;}
  | shift arith_expr shift_arith     {cout << "shift_arith2" << endl;if($2->type == $3->type){strcpy($$->type,$2->type);string t = new_temporary(); emitt($3->temp_op,$2->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,$1->temp_op); strcpy($$->temp_var,t.c_str());$$->i_number = $2->i_number;} else{cout<<"error"<<endl;}}
  ;

shift: GTGT {cout << "shift1" << endl; strcpy($$->temp_op,">>");}
  | LTLT  {cout << "shift2" << endl; strcpy($$->temp_op,"<<");}
  ;
...
arith_expr: term {
    ...
    | term op_term {
       ...
       if(allowed_types($1->type,$2->type)) {
        ...
        string s2 = " "; string s1= $1->temp_var + s2 + $2->temp_op + s2  + $2->temp_var; strcpy($$->temp_var,s1.c_str());
        //string t = new_temporary(); emitt($2->temp_op,$1->temp_var,$2->temp_var,t,-1,backtrue[(backtrue.size())-1]);strcpy($$->temp_var,t.c_str());
        $$->i_number = $1->i_number;
    }
  ;
...
op_term: AddOrSub term {
        cout << "op_term1" << endl;
        strcpy($$->type, $1->type);
        strcpy($$->temp_var,$2->temp_var); strcpy($$->temp_op,$1->temp_op);
        $$->i_number = $2->i_number;
    }
    | AddOrSub term op_term {
       ...
       if(allowed_types($2->type,$3->type)) {
        ...
        string t = new_temporary(); emitt($3->temp_op,$2->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_op,$1->temp_op); strcpy($$->temp_var,t.c_str());
        $$->i_number = $2->i_number;
    }
    ;

AddOrSub: ADD {cout << "AddOrSub1" << endl; strcpy($$->temp_op,"+");}
  | SUB {cout << "AddOrSub2" << endl;strcpy($$->temp_op,"-");}
  ;

term: factor {cout << "term1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;}
...
op_factor_plus: op_factor {cout << "op_factor_plus1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number; }
...
op_factor: MULTIPLY factor  {cout << "op_factor1" << endl;strcpy($$->type,$2->type);
         strcpy($$->temp_op,"*"); strcpy($$->temp_var,$2->temp_var); 
         $$->i_number = $2->i_number;
    }
  ...
  | PERCENT factor          {cout << "op_factor3" << endl;strcpy($$->type,$2->type);
        strcpy($$->temp_op,"%"); strcpy($$->temp_var,$2->temp_var);  $$->i_number = $2->i_number;   }
...
factor: ADD factor          {cout << "factor1" << endl;strcpy($$->type,$2->type);strcpy($$->temp_op,"add"); strcpy($$->temp_var,$2->temp_var);$$->i_number = $2->i_number;}
  | SUB factor              {cout << "factor2" << endl;strcpy($$->type,$2->type);strcpy($$->temp_op,"sub"); strcpy($$->temp_var,$2->temp_var);$$->i_number = $2->i_number;}
  | BITWISE_OPER_NOT factor {cout << "factor3" << endl;strcpy($$->type,$2->type);strcpy($$->temp_op,"bitwise_not"); strcpy($$->temp_var,$2->temp_var);$$->i_number = $2->i_number;}
  | power                   {cout << "factor4" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);}
  ;

power: atom_expr              {cout << "power1" << endl;strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;}       
  | atom_expr DOUBLE_MULTIPLY factor {
    ...
    if(allowed_types($1->type,$3->type)){
       strcpy($1->type, $3->type);
        ...
        string t = new_temporary(); emitt("*",$1->temp_var,$3->temp_var,t,-1,backtrue[(backtrue.size())-1]); strcpy($$->temp_var,t.c_str());
        $$->i_number = $1->i_number;
  } 
  ;
 // check
atom_expr: atom {strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number; if(backtrue.size()== 0 ){backtrue.push_back(0);}}
    | atom trailer {
        if(backtrue.size() == 0 ){backtrue.push_back(0);}
        cout << "trailer_star" << endl;
        ...
        string tem = $1->temp_var;string tok = $2->temp_var;
        string s = "*(" + tem  + "+" + tok + "*" + to_string(get_siz($2->type)) + ")"; 
        cout << s << endl;
        strcpy($$->temp_var,s.c_str());$$->i_number = $1->i_number;
    }
    ;

atom: LEFT_SQUARE_BRACE RIGHT_SQUARE_BRACE     {cout << "atom02" << endl;}                 
    | LEFT_CURLY RIGHT_CURLY        {cout << "atom03" << endl;}                            
    | LEFT_PAR testlist_comp RIGHT_PAR {cout << "atom04" << endl; strcpy($$->type, $2->type);}
    | LEFT_SQUARE_BRACE testlist_comp RIGHT_SQUARE_BRACE {cout << "atom05" << endl;strcpy($$->type, $2->type); string s1 = $2->token; string s = "["+ s1 + "]"; strcpy($$->token,s.c_str());
        strcpy($$->temp_var,new_temporary().c_str());emitt("",s.c_str(),"",$$->temp_var,-1,backtrue[(backtrue.size())-1]);$$->i_number = inst_num-1;
        }
    ...
    | INT { 
       ...
       strcpy($$->type,"int");
       strcpy($$->temp_var,new_temporary().c_str());emitt("",$1->token,"",$$->temp_var,-1,backtrue[(backtrue.size())-1]);$$->i_number = inst_num-1;
    }
    | FLOAT {
        cout << "atom2" << endl;
       strcpy(($$)->type,"float");
    ...
    | NAME {
        ...
        // }
        if($1->token == "__name__"){ strcpy($$->type,"str");}
        else{
        if(entry_lookup(current,$1->token)!=NULL){
            cout<<"entered"<<endl;
        ...
        char char_array[1000]; 
        strcpy(char_array, s1.c_str()); 
         if(entry_lookup(current,char_array)!=NULL){
            cout<<"entered"<<endl;
            symbol_entry* temp_entry = entry_lookup(current,char_array);
            strcpy($$->temp_var,temp_entry->temp_var);
    ...
    | NAME v {
        cout << "atom7" <<yylineno<<endl;
        for(int i=0;i<arguments.size();i++) { 
        string s1 = get<0>(arguments[i]);
        char char_array[1000]; 
        strcpy(char_array, s1.c_str()); 
        if(entry_lookup(current,char_array)){
        symbol_entry* temp_entry = entry_lookup(current,char_array);
        cout << "\n" << "\n" << "\n"<<temp_entry->offset<<temp_entry->token<<temp_entry->temp_var<<"\n"<<"fejiefjif"<<endl;
        emitt("","pushparam","",temp_entry->temp_var,-1,backtrue[(backtrue.size())-1]);
        emitt("",temp_entry->temp_var,"",get<0>(arguments[i]),-1,backtrue[(backtrue.size())-1]);
            ...
            strcpy($$->type,"int");
            string t = new_temporary();
            emitt("",get<0>(arguments[i]),"",t.c_str(),-1,backtrue[(backtrue.size())-1]);
            emitt("","pushparam","",t.c_str(),-1,backtrue[(backtrue.size())-1]);
        } }
        cout << "epfeifef"<<endl;
       // symbol_table* temp_table = func_table_lookup(global,$1->token);
        //cout << temp_table->token << "      oehfefe"<< endl;
         if(entry_lookup(current,$1->token)!=NULL){
            cout<<"entered"<<endl;
            strcpy($$->type,(entry_lookup(current,$1->token))->type);
         }
        emitt("call",$1->token,"","",-1,backtrue[(backtrue.size())-1]);
        string t = new_temporary();
        emitt("","returnpop","",t,-1,backtrue[(backtrue.size())-1]);
...
string_plus: STRING         {cout << "string_plus1" << endl;strcpy($$->type,"string");strcpy($$->temp_var,new_temporary().c_str());emitt("",$1->token,"",$$->temp_var,-1,backtrue[(backtrue.size())-1]);$$->i_number = inst_num-1;}
    | STRING string_plus    {cout << "string_plus2" << endl;strcpy($$->type,"string");strcpy($$->temp_var,new_temporary().c_str());emitt("",$1->token,"",$$->temp_var,-1,backtrue[(backtrue.size())-1]);$$->i_number = inst_num-1;}
    ;
testlist_comp: namedexprtest_or_starexpr comma_namedexprtest_or_starexpr_star comma_brack  {
        ...
        string s1 = $1->token; string s2 = $2->token; string s3 = $3->token;
        string s = s1 + s2;
        strcpy($$->token,s.c_str()); strcpy($$->temp_var,$1->temp_var);
    }
...
namedexprtest_or_starexpr: test {strcpy($$->type, $1->type);cout << "namedexprtest1" << endl;
         strcpy($$->temp_var,$1->temp_var); strcpy($$->token,$1->token); cout << $1->token << "     grgrgegeheh"<< endl;    }
    | star_expr {strcpy($$->type, $1->type);cout << "namedexprtest2" << endl;strcpy($$->temp_var,$1->temp_var); strcpy($$->token,$1->token); }
    ;
comma_namedexprtest_or_starexpr_star: COMMA namedexprtest_or_starexpr  {
        ...
        string s = $2->token;
        string s2 = "," + s;
        strcpy($$->token,s2.c_str()); strcpy($$->temp_var,$2->temp_var);
    }
    ...
    ;
trailer: LEFT_PAR arglist RIGHT_PAR {
    strcpy($$->type,$2->type);
    cout << "trailer1" << endl;}
    | LEFT_SQUARE_BRACE subscriptlist RIGHT_SQUARE_BRACE {
       strcpy($$->type,$2->type);  
       strcpy($$->temp_var,$2->temp_var);
       cout << "trailer2" << endl;}
...
subscriptlist: subscript comma_subscript_star comma_brack {
    ...
    | subscript comma_brack {
        strcpy($$->type,$1->type);
        cout << "subscriptlist2" << endl;
        strcpy($$->temp_var,$1->temp_var);
        $$->no_args++;
    }
    ;
comma_subscript_star:COMMA subscript {
    strcpy($$->type,$2->type);cout << "comma_subscript_star1" << endl; $$->no_args++;}
    ...
    ;
subscript: test {strcpy($$->type,$1->type);cout << "subscript1" << endl;}
...
testlist: test comma_test comma_brack {
       cout<<"testlist1"<<endl;
       strcpy($$->type,$1->type);strcpy($$->temp_var,$2->temp_var);
    }
    | test comma_brack {  
        cout<<"testlist2"<<endl;
        $$=new nonterminal();
        strcpy($$->type,$1->type);strcpy($$->temp_var,$1->temp_var);
        $$->no_args=1; 
    }
    ;
...
classdef_def: CLASS NAME COLON { 
    cout << "classdef1" << endl;
        ...
        emitt("begin class",$2->token,"","",-1,backtrue[(backtrue.size())-1]);
        $$->i_number = inst_num-1;
    }
...
argument: test sync_comp_for {
    ...
    | test { cout << "argument5" << endl;strcpy($$->temp_var,$1->temp_var);$$->i_number = $1->i_number;strcpy($$->type,$1->type);}
    ;
...
comma_test: COMMA test      {
        cout << "comma_test1" << endl;
        strcpy($$->type, $2->type);
        strcpy($$->temp_var,$2->temp_var);
    }
    | COMMA test comma_test {
        cout << "comma_test2" << endl;
        if(!strcmp($2->type, $3->type)) {
            strcpy($$->type, $2->type);
        } else {
            cout << "Arguments are of different type at: " << yylineno << endl;
        }
        strcpy($$->type, $2->type);
        strcpy($$->temp_var,$2->temp_var);
    }
...
test_brack:      {
    ...
    | test          {cout << "test_brack2" << endl;strcpy($$->temp_var,$1->temp_var); $$->i_number = $1->i_number;strcpy($$->type,$1->type);}
    ;
